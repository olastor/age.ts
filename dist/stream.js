...
export function decryptSTREAM(key, ciphertext) {
    ...
    while (ciphertext.length > chunkSizeWithOverhead) {
        const chunk = chacha20poly1305(key, streamNonce).decrypt(ciphertext.subarray(0, chunkSizeWithOverhead));
...
export function encryptSTREAM(key, plaintext) {
    ...
    while (plaintext.length > chunkSize) {
        const chunk = chacha20poly1305(key, streamNonce).encrypt(plaintext.subarray(0, chunkSize));
